...
def analyze_audio(audio_path: str) -> dict:
    try:
        ...
        segments, info = model.transcribe(audio_path, word_timestamps=True, vad_filter=True, language="en")
        ...
        filler_pattern = re.compile(r"\b(" + "|".join(re.escape(fw) for fw in filler_words_list) + r")\b", re.IGNORECASE)
        ...
        for segment in segments:
            ...
            if hasattr(segment, 'words') and segment.words is not None:
                for word_info in segment.words:
                    ...
                    if filler_pattern.search(word_lower):
                        ...
                        for filler in filler_words_list:
                            if re.search(r"\b" + re.escape(filler) + r"\b", word_lower, re.IGNORECASE):
                ...
                print(f"Warning: Word-level timestamps not available for segment: '{segment.text}'")
