...
async def notify_callback(callback_url: str, interview_id: str, result_data: dict):
    try:
        ...
        async with httpx.AsyncClient(
            timeout=120.0, transport=httpx.AsyncHTTPTransport(local_address="0.0.0.0")
        ) as client:
...
async def notify_task_status(
    task_status_callback_url: str, interview_id: str, status: str
):
    try:
        async with httpx.AsyncClient(timeout=30.0) as client:
            response = await client.patch(
                f"{task_status_callback_url}",
                json={"interview_id": interview_id, "status": status},
            )
            response.raise_for_status()
...
async def notify_worker_status(
    worker_status_callback_url: str, interview_id: str, status: str
):
    try:
        async with httpx.AsyncClient(timeout=30.0) as client:
            response = await client.patch(
                f"{worker_status_callback_url}",
                json={"interview_id": interview_id, "status": status},
            )
            response.raise_for_status()
...
async def submit_processing_job(
    ...
    task_status_callback_url: str,
    worker_status_callback_url: str,
):
    try:
        ...
        interview_result = await process_interview(
            interview_id,
            {"videoUrl": video_url, "timestamps": timestamps, "questions": questions},
        )
        ...
        try:
            ...
            await notify_task_status(
                task_status_callback_url, interview_id, "PROCESSED"
            )
            await notify_worker_status(
                worker_status_callback_url, interview_id, "AVAILABLE"
            )
        except Exception as e:
            logger.error(
                f"Error updating task and worker status for job {interview_id}: {str(e)}"
            )

    except Exception as e:
        await notify_task_status(
            task_status_callback_url, interview_id, "FAILED_PROCESSING"
        )
        await notify_worker_status(
            worker_status_callback_url, interview_id, "AVAILABLE"
        )
